...
class ChatOption extends React.Component {
  ...
  sendMessage = () => {
    const { mystore, selectedProduct } = this.props;
    let product_url = `http://${window.location.hostname}`;
    if (selectedProduct) {
      product_url += `/product/${selectedProduct.id}`;
    }
    console.log(product_url);

    if ( !this.validate())
      return;
    const message = `Hi, I'm ${this.state.name}. I'd like to inquire about the product.\n ${product_url}`;
    const url = "https://api.whatsapp.com/send?phone=" + mystore.phone + "&text=" + message;
    window.location = url;
  }
  ...
  render () {
    ...
    return (
      <div>
        ...
        >
          Fluxstore is a WhatsApp Commerce platform.* It has nothing to do with
          WhatsApp Inc.
        </Typography>
        <TextField
...
export default withRouter(
  withStyles(myStyles(theme))(connect(mapStateToProps, {})(ChatOption))
)
